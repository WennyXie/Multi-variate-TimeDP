    ...
    def forward(self, x, context=None, mask=None):
        ...
        if exists(mask):
            ...
            if self.use_pam:
                ...
                # Safety fix: if ALL prototypes are masked (mask <= 0), keep the one with highest value
                # This prevents softmax(-inf, -inf, ...) = NaN
                # Fully vectorized - no GPU-CPU sync
                all_masked = (mask <= 0).all(dim=-1, keepdim=True)  # (b*h, 1, 1)
                max_mask_idx = mask.argmax(dim=-1, keepdim=True)  # (b*h, 1, 1)
                preserve_mask = torch.zeros_like(mask_of_mask).scatter_(-1, max_mask_idx, 1.0)
                # Only modify mask_of_mask for all-masked samples (vectorized)
                mask_of_mask = torch.where(
                    all_masked & (preserve_mask == 1.0),
                    torch.zeros_like(mask_of_mask),  # unblock the max position
                    mask_of_mask
                )
