...
def init_model_data_trainer(parser):
    ...
    if opt.uncond:
        ...
        if opt.use_pam:
            # Phase-2 extension: allow additive prototype sources (e.g. VQ-VAE codebook) via a wrapper target.
            # Default behavior must remain identical:
            #   - for baseline configs, target is DomainUnifiedPrototyper (we set it again, no change).
            #   - for any other non-router targets, we keep the previous behavior of forcing DomainUnifiedPrototyper.
            #   - ONLY when the config explicitly sets ProtoRouter, we preserve it.
    ...
    # data
    # Handle both old (data_path_dict) and new (data_dir) data formats
    if 'data_path_dict' in config.data.params and config.data.params.data_path_dict is not None:
    ...
    data.prepare_data()
    data.setup()
    # Note: multivariate input is now supported (e.g., input_channels=6 for nuScenes)
