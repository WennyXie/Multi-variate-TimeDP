...
def is_multivariate_data(data_module):
    """Check if the data module is for multivariate data."""
    return hasattr(data_module, 'input_channels') and data_module.input_channels > 1
...
def test_model_with_dp(model, data, trainer, opt, logdir):
    ...
    multivariate = is_multivariate_data(data)
    
    for dataset in data.norm_train_dict:
        if multivariate:
            # Multivariate: data is already (N, F, T) format
            dataset_arr = data.norm_train_dict[dataset]
            n_samples = min(num_dp, len(dataset_arr))
            indices = np.random.choice(len(dataset_arr), size=n_samples, replace=False)
            dataset_samples = dataset_arr[indices]  # (n_samples, F, T)
            
            # Model expects (B, F, T) for conditioning
            x = torch.tensor(dataset_samples).to('cuda').float()
        else:
        ...
        if multivariate:
            # For multivariate: keep (N, F, T) format
            generated_data = np.vstack(all_gen)  # (N, F, T)
        else:
        ...
        if not multivariate:
            # Only run univariate metrics for univariate data
            ...
            # For multivariate, just report basic stats
            print(f"\n{dataset} generation stats:")
            print(f"  Shape: {generated_data.shape}")
            print(f"  Mean: {generated_data.mean():.4f}")
            print(f"  Std: {generated_data.std():.4f}")
            all_metrics[f'{dataset}_shape'] = generated_data.shape
            all_metrics[f'{dataset}_mean'] = float(generated_data.mean())
            all_metrics[f'{dataset}_std'] = float(generated_data.std())
...
def test_model_uncond(model, data, trainer, opt, logdir):
    ...
    for dataset in data.norm_train_dict:            
        ...
        if not multivariate:
            ...
            print(f"\n{dataset} uncond generation stats:")
            print(f"  Shape: {generated_data.shape}")
            print(f"  Mean: {generated_data.mean():.4f}")
            print(f"  Std: {generated_data.std():.4f}")
            all_metrics[f'{dataset}_uncond_shape'] = generated_data.shape
            all_metrics[f'{dataset}_uncond_mean'] = float(generated_data.mean())
            all_metrics[f'{dataset}_uncond_std'] = float(generated_data.std())
